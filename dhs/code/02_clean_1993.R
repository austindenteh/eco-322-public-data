# =============================================================================
# 02_clean_1993.R — Clean and Analyze Ghana DHS 1993
# =============================================================================
# This script loads the harmonized 1993 working dataset, creates analysis
# variables for demographics, education, religion, ethnicity, and region,
# and produces descriptive statistics.
#
# Ghana DHS 1993 (DHS Phase III)
...
#
# Input:  output/ghana_dhs_1993_working.rds (or .dta from Stata)
# Output: output/ghana_dhs_1993_analysis.rds
#         output/ghana_dhs_1993_analysis_from_R.dta
# =============================================================================
...
sink(file.path(out_dir, "02_clean_1993_R_log.txt"), split = TRUE)
cat("============================================================\n")
cat("Ghana DHS 1993 -- Clean and Analyze (R)\n")
cat("============================================================\n\n")

# Load data (prefer .rds from R, fall back to .dta from Stata)
rds_file <- file.path(out_dir, "ghana_dhs_1993_working.rds")
dta_file <- file.path(out_dir, "ghana_dhs_1993_working.dta")
...
if (file.exists(rds_file)) {
  ...
  stop("Working dataset not found. Run 01_load_1993 first.")
}
...
# NOTE: Literacy (v155) is NOT available in 1993.
# The literate variable cannot be constructed for this wave.
cat("  NOTE: Literacy variable (v155) not available in 1993 -- skipping.\n")
...
cat("  NOTE: Health insurance (v481/v481c) not available in 1993.\n")
cat("  NHIS was not created until 2003. Skipping insurance section.\n")

# ==========================================================================
# SECTION 6: Region (10 regions in 1993)
# ==========================================================================

cat("--- Section 6: Region ---\n")

region_labels_1993 <- c(
...
dhs <- dhs %>%
  mutate(
    region_name = region_labels_1993[as.character(region)],
...
cat("  NOTE: Wealth index (v190) not available in 1993. Skipping wealth section.\n")
...
cat("\n============================================================\n")
cat("DESCRIPTIVE STATISTICS -- Ghana DHS 1993\n")
cat("============================================================\n")
...
# Summary means (excluding literate, poor, insurance — not available in 1993)
cat("\n--- 8h. Summary Means ---\n")
...
saveRDS(dhs, file.path(out_dir, "ghana_dhs_1993_analysis.rds"))
write_dta(dhs %>% select(-region_name), file.path(out_dir, "ghana_dhs_1993_analysis_from_R.dta"))

cat(sprintf("\n============================================================\n"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "ghana_dhs_1993_analysis.rds")))
cat(sprintf("  N = %d\n", nrow(dhs)))
cat("============================================================\n")
