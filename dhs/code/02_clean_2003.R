# =============================================================================
# 02_clean_2003.R — Clean and Analyze Ghana DHS 2003
# =============================================================================
...
#
# Input:  output/ghana_dhs_2003_working.rds (or .dta from Stata)
# Output: output/ghana_dhs_2003_analysis.rds
#         output/ghana_dhs_2003_analysis_from_R.dta
# =============================================================================
...
sink(file.path(out_dir, "02_clean_2003_R_log.txt"), split = TRUE)
cat("============================================================\n")
cat("Ghana DHS 2003 -- Clean and Analyze (R)\n")
cat("============================================================\n\n")

# Load data (prefer .rds from R, fall back to .dta from Stata)
rds_file <- file.path(out_dir, "ghana_dhs_2003_working.rds")
dta_file <- file.path(out_dir, "ghana_dhs_2003_working.dta")
...
if (file.exists(rds_file)) {
  ...
  stop("Working dataset not found. Run 01_load_2003 first.")
}
...
dhs <- dhs %>%
  mutate(
    ...
    literate = case_when(
      educ_level == 3                    ~ 1L,   # higher education
      literacy %in% c(1, 2)              ~ 1L,   # can read part/whole
      literacy == 0                      ~ 0L,   # cannot read
...
# ==========================================================================
# SECTION 5: Region (10 regions in 2003)
# ==========================================================================

cat("--- Section 5: Region ---\n")

region_labels_2003 <- c(
...
dhs <- dhs %>%
  mutate(
    region_name = region_labels_2003[as.character(region)],
...
cat("\n============================================================\n")
cat("DESCRIPTIVE STATISTICS -- Ghana DHS 2003\n")
cat("============================================================\n")
...
cat("\n--- 7g. Wealth ---\n")
print(table(Wealth = dhs$wealth_index, Female = dhs$female, useNA = "ifany"))
cat("\n")
print(table(Poor = dhs$poor, Female = dhs$female, useNA = "ifany"))

cat("\n--- 7h. Region ---\n")
print(table(Region = dhs$region, Female = dhs$female, useNA = "ifany"))
cat("\n")
print(table(Northern = dhs$northern, Female = dhs$female, useNA = "ifany"))
...
cat("\n--- 7j. Summary Means by Sex ---\n")
summ_vars <- c("urban", "age_years", "educ_none", "educ_primary", "educ_second",
               "educ_higher", "literate", "in_union", "employed", "poor")
...
saveRDS(dhs, file.path(out_dir, "ghana_dhs_2003_analysis.rds"))
write_dta(dhs %>% select(-region_name), file.path(out_dir, "ghana_dhs_2003_analysis_from_R.dta"))

cat(sprintf("\n============================================================\n"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "ghana_dhs_2003_analysis.rds")))
cat(sprintf("  N = %d\n", nrow(dhs)))
cat("============================================================\n")
