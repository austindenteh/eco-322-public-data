# =============================================================================
# 02_clean_2022.R — Clean and Analyze Ghana DHS 2022
# =============================================================================
...
#
# Input:  output/ghana_dhs_2022_working.rds (or .dta from Stata)
# Output: output/ghana_dhs_2022_analysis.rds
#         output/ghana_dhs_2022_analysis_from_R.dta
# =============================================================================
...
sink(file.path(out_dir, "02_clean_2022_R_log.txt"), split = TRUE)
cat("============================================================\n")
cat("Ghana DHS 2022 -- Clean and Analyze (R)\n")
cat("============================================================\n\n")

rds_file <- file.path(out_dir, "ghana_dhs_2022_working.rds")
dta_file <- file.path(out_dir, "ghana_dhs_2022_working.dta")
...
if (file.exists(rds_file)) {
  ...
  stop("Working dataset not found. Run 01_load_2022 first.")
}
...
# ==========================================================================
# SECTION 6: Region (16 regions in 2022 — post-2019 reorganization)
# ==========================================================================

cat("--- Section 6: Region ---\n")

region_labels_2022 <- c(
  "1" = "Western", "2" = "Central", "3" = "Greater Accra", "4" = "Volta",
  "5" = "Eastern", "6" = "Ashanti", "7" = "Western North", "8" = "Ahafo",
  "9" = "Bono", "10" = "Bono East", "11" = "Oti", "12" = "Northern",
  "13" = "Savannah", "14" = "North East", "15" = "Upper East", "16" = "Upper West"
)

dhs <- dhs %>%
  mutate(
    region_name = region_labels_2022[as.character(region)],
    northern    = as.integer(region %in% c(12, 13, 14, 15, 16))
  )
...
cat("\n============================================================\n")
cat("DESCRIPTIVE STATISTICS -- Ghana DHS 2022\n")
cat("============================================================\n")
...
saveRDS(dhs, file.path(out_dir, "ghana_dhs_2022_analysis.rds"))
write_dta(dhs %>% select(-region_name), file.path(out_dir, "ghana_dhs_2022_analysis_from_R.dta"))

cat(sprintf("\n============================================================\n"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "ghana_dhs_2022_analysis.rds")))
cat(sprintf("  N = %d\n", nrow(dhs)))
cat("============================================================\n")
