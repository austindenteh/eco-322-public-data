...
#
# Purpose: Standalone low-memory NHIS loader. Loads NHIS adult and child files,
#          harmonizes names across the 2019 redesign, and keeps only the
#          starter variables needed by 02_clean_and_analyze.R plus any
...
#
# Usage:   Run from nhis/, nhis/code/, from the repo root, or set
...
cat(sprintf("Saved: %s\n", file.path(out_dir, "nhis_adult.rds")))
if (write_dta_export) {
  tryCatch({
    write_dta(adult, file.path(out_dir, "nhis_adult_from_r.dta"))
    cat(sprintf("Saved: %s\n", file.path(out_dir, "nhis_adult_from_r.dta")))
  }, error = function(e) cat(paste0("Could not save adult .dta: ", e$message, "\n")))
} else {
  cat("Skipped optional Stata export. Set write_dta_export <- TRUE to create nhis_adult_from_r.dta.\n")
}

# --- Child ---
child <- child %>% arrange(srvy_yr, hhx)
saveRDS(child, file.path(out_dir, "nhis_child.rds"))
cat(sprintf("Saved: %s\n", file.path(out_dir, "nhis_child.rds")))
if (write_dta_export) {
  tryCatch({
    write_dta(child, file.path(out_dir, "nhis_child_from_r.dta"))
    cat(sprintf("Saved: %s\n", file.path(out_dir, "nhis_child_from_r.dta")))
  }, error = function(e) cat(paste0("Could not save child .dta: ", e$message, "\n")))
} else {
  cat("Skipped optional Stata export. Set write_dta_export <- TRUE to create nhis_child_from_r.dta.\n")
}
...
#
# 2. LOW-MEMORY BUILDS:
#    This optional script is standalone and always keeps the starter variables
#    plus requested extras. Use extra_vars for stable additional variable names.
#    Use extra_var_families for aliases that changed names across years, for
#    example:
#      extra_var_families <- list(
